...
/* static */
bool Protocol::encode(char *dest, size_t &off, size_t max,
...
bool Protocol::decode(char *src, size_t &off, size_t max,
                      std::string &id, int64_t &size) {
  folly::ByteRange br((uint8_t *)(src+off), max);
  size_t idLen = folly::decodeVarint(br);
  if (idLen + off + 1 >= max) {
    LOG(ERROR) << "Not enough room with " << max
               << " to decode " << idLen << " at " << off;
    return false;
  }
